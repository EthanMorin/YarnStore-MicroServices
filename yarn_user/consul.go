...
func Register() {
	...
	serviceId := "go-user-service:" + getHostname()
	...
	tags := []string{
		"traefik.enable=true",
		fmt.Sprintf("traefik.http.routers.%s.rule=PathPrefix(`/user`)", getHostname()),
	}
	registeration := &consulapi.AgentServiceRegistration{
		...
		Check: &consulapi.AgentServiceCheck{
			HTTP:     fmt.Sprintf("http://%s:%v/user/check", address, port),
			Interval: "10s",
			Timeout:  "30s",
		},
	}
